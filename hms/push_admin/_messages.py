...
class Message(object):
    ...
    def __init__(
        ...
        MessageValidator.check_message(
            data, notification, android, apns, web_push, token, topic, condition
        )
        self.data = data
...
class AndroidConfig(object):
    ...
    def __init__(
        ...
        MessageValidator.check_android_config(
            collapse_key, urgency, ttl, bi_tag, fast_app_target, notification, data
        )
...
class MessageValidator(object):
    ...
    @classmethod
    def check_message(
        cls, data, notification, android, apns, web_push, token, topic, condition
    ):
        """
        ...
        cls.check_type(
            android, AndroidConfig, "android must be an instance of AndroidConfig class"
        )
    ...
    def check_android_config(
        cls, collapse_key, urgency, ttl, bi_tag, fast_app_target, notification, data
    ):
        # collapse_key
    ...
    def check_android(
        ...
        # use_default_vibrate
        cls.check_boolean(
            hint="AndroidNotification.use_default_vibrate", value=use_default_vibrate
        )
        # use_default_light
        cls.check_boolean(
            hint="AndroidNotification.use_default_light", value=use_default_light
        )
        # vibrate_config
        cls.check_string_list(
            label="AndroidNotification.vibrate_config", value=vibrate_config
        )
        # visibility
        cls.assert_string_values(
        ...
        # foreground_show
        cls.check_boolean(
            hint="AndroidNotification.foreground_show", value=foreground_show
        )
    ...
    def check_badge_notification(cls, add_num, set_num, clazz):
        ...
        cls.check_number_span(
            label="AndroidBadgeNotification.add_num", value=add_num, min=0, max=100
        )
        # set_num must be int
        cls.check_number_span(
            label="AndroidBadgeNotification.set_num", value=set_num, min=0, max=100
        )
        # clazz
    ...
    def check_light_settings(cls, color, light_on_duration, light_off_duration):
        ...
        cls.check_string(
            hint="AndroidLightSettings.light_on_duration", value=light_on_duration
        )
        cls.check_string(
            hint="AndroidLightSettings.light_off_duration", value=light_off_duration
        )
    ...
    def check_webpush_config(cls, headers, data, notification, hms_options):
        # headers
        cls.check_type(
            headers, WebPushHeader, "headers must be an instance of WebPushHeader"
        )
    ...
    def check_webpush_notification(
        ...
        cls.check_type_list(
            "WebPushNotificationAction.actions", actions, WebPushNotificationAction
        )
    ...
    def check_apns_config(cls, headers=None, payload=None, apns_hms_options=None):
        ...
        cls.check_type(
            payload, APNsPayload, "payload must be an instance of APNsPayload"
        )
        cls.check_type(
            apns_hms_options,
            APNsHMSOptions,
